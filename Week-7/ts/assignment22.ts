...
function main() {
    ...
    let sumArray: number[][] = addArray(limit as number, array1 as number[][], array2 as number[][]);
    displayArray(limit as number, sumArray as number[][]);
}
...
function addArray(limit: number, array1: number[][], array2: number[][]) {
    let array : number[][] = [];
    for (let i: number = 0; i < limit; i++) {
        let values: number[] = [];
        for (let j: number = 0; j < limit; j++) {
